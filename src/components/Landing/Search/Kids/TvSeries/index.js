...
class KidsTvSeries extends Component {
  ...
  componentWillMount() {
    ...
    data.map((item, index)=>{
      ...
      getKidsImages(kidsEndpoints.kidsSeriesImages, queryParams, index, 'results', 'tvSeries', 'kids');
    });
  ...
  loadMore = () => {
    ...
    const {filter, countFilter} = this.state;
      ...
      loadMoreContent(showEndpoints.searchAssets,formDataGenerator(data), 'tvSeries', 'kids', ()=>{
...
const mapStateToProps = (state, ownProps) => {
  return {
    tvSeries: state.search.kids.results.tvSeries,
