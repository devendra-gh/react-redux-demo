...
class KidsEpisodes extends Component {
  constructor(props) {
    super(props);
  ...
  componentWillMount() {
    ...
    if(episodes.count){
      ...
      episodes.data.map((item,key) => {
        searchAssestInfoCalls(endpoints.searchAssets, formDataGenerator(searchActionSearchAssetsKidsFilter(clone(FILTER), item)), 'episodes', key, 'kids');
      });
  ...
  loadMore = () => {
    ...
    if((countFilter + 1) % 2 == 1 && countFilter!==1){
      ...
      loadMoreContent(showEndpoints.searchAssets,formDataGenerator(data), 'episodes', 'kids', ()=>{
        const {episodes} = this.state;
        const {searchAssestInfoCalls} = this.props;
  ...
  render() {
    const {episodes: {count, data}, countFilter, load} = this.state;
      ...
      <div>
        {
          count ?
            <div className='search-category'>
              <span className='search-type'>KIDS {headingCreator('videos', count)}</span>
...
const mapStateToProps = (state, ownProps) => {
  return {
    episodes: state.search.kids.results.episodes,
    keyword: ownProps.keyword,
  };
};

KidsEpisodes.propTypes = {
...
KidsEpisodes.contextTypes = {
  router: React.PropTypes.object.isRequired,
};

export default connect(mapStateToProps, mapDispatchToProps)(KidsEpisodes);
